...
void ssd1306_toggle_border(ssd1306_t *ssd, bool state) {
  ssd1306_hline(ssd, 0, ssd->width - 1, 0, state);                   // Linha superior
  ssd1306_hline(ssd, 0, ssd->width - 1, ssd->height - 1, state);     // Linha inferior
  ssd1306_vline(ssd, 0, 0, ssd->height - 1, state);                  // Linha esquerda
  ssd1306_vline(ssd, ssd->width - 1, 0, ssd->height - 1, state);     // Linha direita
...
void ssd1306_draw_pixel(ssd1306_t *ssd, int x, int y) {
  if (x < 0 || x >= ssd->width || y < 0 || y >= ssd->height) return; // Verifica se está dentro dos limites

  int byte_index = x + (y / 8) * ssd->width; // Calcula a posição no buffer
  int bit_mask = 1 << (y % 8); // Define a máscara para o bit correto

  ssd->ram_buffer[byte_index] |= bit_mask; // Ativa o bit correspondente no buffer
}
